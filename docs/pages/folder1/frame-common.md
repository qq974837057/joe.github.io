...
- 不同（新）
  ...
  | 数据绑定 | 单向，通过 this.$setState() | 双向，响应式系统 |
...
- innerHTML 的总计算量不管是 js 计算还是 DOM 操作都是和整个界面的大小相关，但 Virtual DOM 的计算量里面，只有 js 计算和界面大小相关，DOM 操作是和数据的变动量相关的，越少变动，操作 DOM 越少。和 DOM 操作比起来，js 计算是极其便宜的（知乎尤大）。
- 其实虚拟 DOM 可以提高开发效率，让我们写代码时减少操作 dom，不一定能带来更好的性能，只能说还不错

### 不使用虚拟 DOM 的 angular

- Angular 是脏检查，任何小变动都有的和 watcher 数量相关的性能代价，性能较差
- 当数据全部改变时，就不吃亏了
