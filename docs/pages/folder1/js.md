...
- 变量提升
  ...
  - 作用域分全局作用域、函数作用域、es6 使用 let/const 创建的块级作用域，作用域链里的变量对象是一层一层 push 的，可以往上查找，但外层访问不到内层的作用域变量对象。而变量提升都是在自己的作用域头部，例如全局作用域 or 函数作用域的头部。
...
- 节流 throttle：连续触发事件，每隔一段时间只执行一次。
  ...
  - 场景：
    ...
    - 缩放场景：监控浏览器调整窗口时，触发 resize，用防抖来让其只触发一次。
...
- 事件监听: 没有正确销毁，如使用监听执行匿名内联函数，无法使用 removeEventListener() 将其删除
  ```js
